...
void
PointToPlanePointSetToPointSetMetricv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::
  GetValueAndDerivative(MeasureType & calculatedValue, DerivativeType & derivative) const
{
  ...
  {
    derivative.SetSize(TFixedPointSet::PointDimension * this->m_FixedTransformedPointSet->GetNumberOfPoints());
  }
  ...
  std::function<void(SizeValueType)> sumNeighborhoodValues =
    [this, &derivative, &threadDerivatives, &threadValues, &ranges, &calculateValue, &numberOfLocalParameters](
      ...
      MovingTransformJacobianType jacobian(TFixedPointSet::PointDimension, numberOfLocalParameters);
      ...
      for (PointIdentifier index = ranges[rangeIndex].first; index < ranges[rangeIndex].second; ++index)
      {
        ...
        /* Obtain jacobian by doing dot product with the normal stored in pixel*/
        float new_jacobian[numberOfLocalParameters] = { 0 };
        for (NumberOfParametersType par = 0; par < numberOfLocalParameters; ++par)
        {
          for (DimensionType d = 0; d < TFixedPointSet::PointDimension; ++d)
          {
            new_jacobian[par] = new_jacobian[par] + jacobian(d, par) * pixel[d];
          }
        }

        /* Use the Jacobian to get the delta */
        for (NumberOfParametersType par = 0; par < numberOfLocalParameters; ++par)
        {
